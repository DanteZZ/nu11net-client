class storage {
    ...
    #__dir = function () {
        return this.#__pth.join(
            global._basedir,
            global.__cfg.get().serversDir,
            global._ws.srvInfo.hash,
            String(global._ws.userInfo.id),
            "interfaces",
            this._id
        );
    };
    ...
    __writeMap = function () {
        ...
        /* Функция которую надо будет переписать под серверную */
        let mp = {};
        if (typeof this.__map == "object") {
            mp = this.__map;
        } // Проверяем чтобы нам не посылали всякую шляпу в MAP

        try {
            this.#__fs.writeFileSync(
                this.#__pth.join(this.#__dir(), "map"),
                JSON.stringify(mp)
            );
            return true;
        } catch (e) {
    ...
    _getMapPath(path) {
        if (!this.__mapped) {
            this.#__err("0x000101", "Unmapped storage");
            return false;
        }
        if (path) {
	...
	*/
    ...
    _mkDirAsync(path, func) {
        // Создать директорию асинхронно
	...
	*/
    ...
    _readDataAsync(path, func) {
        ...
        if (map && map.t == 1) {
            ...
            if (this.#__fs.existsSync(p)) {
                // Если физически файл существует
                this.#__fs.readFile(p, "utf-8", function (err, data) {
                    func(data);
                }); // Возвращаем его
            } else {
                /*
    ...
    __saveBuffer = (path, data) =>
        new Promise((res, rej) => {
            const dirPath = this.#__pth.join(path, "..");
            if (!this.#__fs.existsSync(dirPath)) {
                // Check Buffer Dir
                this.#__fse.ensureDirSync(dirPath);
            }
            this.#__fs.writeFile(
                path,
                Buffer.from(data),
    ...
    __initInterface = async (mapType) => {
        const pmap = this.#__pth.join(this.#__dir(), "map");

        if (!this.#__fs.existsSync(this.#__dir())) {
            // Check storageDir
            this.#__fse.ensureDirSync(this.#__dir());
        }
        ...
        if (!this.#__fs.existsSync(pmap)) {
            ...
            for (let i in files) {
                ...
                await this.__saveBuffer(
                    this.#__pth.join(this.#__dir(), name.charAt(0), name),
                    data
                );
            }
        }
