class OGEngine {
  ...
  loadProject(path, callback) {
    ...
    this.project = this.requireUncached(path + "project.js"); //this.loadObjectAsync(path+"project.js");
    this.project.path = path;

    this._rl.onReady(function () {
  ...
  onResourceLoaded(callback) {
    ...
    if (this.project.FPSLimit !== undefined) {
      this.FPSLimit = this.project.FPSLimit;
    }
    this._em.emit("load");
...
class ResourceLoader {
  ...
  isReady() {
    ...
    for (var k in global.resourceCache) {
      if (global.resourceCache.hasOwnProperty(k) && !global.resourceCache[k]) {
        ready = false;
      }
    }
    return ready;
